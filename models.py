...
class Attention(nn.Module):
  ...
  def forward(self, inputs):
    ...
    results = self.dense1(inputs) # results.shape = (batch, 739, 3 * channel)
    b, s, _ = results.shape
...
class ABlock(nn.Module):
  def __init__(self, **kwargs):
    ...
    self.gelu = nn.GELU()
    self.layernorm1 = nn.LayerNorm([739, self.channel,])
    self.layernorm2 = nn.LayerNorm([739, self.channel,])
    self.dropout0 = nn.Dropout(self.drop_rate)
    self.dropout1 = nn.Dropout(self.drop_rate)
    self.dropout2 = nn.Dropout(self.drop_rate)
    self.atten = Attention(**kwargs)
  def forward(self, inputs):
    # inputs.shape = (batch, 739, channel)
    # attention
    skip = inputs
    results = self.layernorm1(inputs) # results.shape = (batch, 739, channel)
    results = self.atten(results) # results.shape = (batch, 739, channel)
    results = self.dropout0(results)
    results = skip + results
    # mlp
    skip = results
    results = self.layernorm2(results)
    results = self.dense1(results) # results.shape = (batch, 739, channel * mlp_ratio)
    results = self.gelu(results)
    results = self.dropout1(results)
    results = self.dense2(results) # results.shape = (batch, 739, channel)
    results = self.dropout2(results)
    results = skip + results
    return results
...
class Extractor(nn.Module):
  def __init__(self, **kwargs):
    ...
    self.layernorm1 = nn.LayerNorm([739, 1,])
    self.layernorm2 = nn.LayerNorm([739, self.hidden_channels])
    self.dropout1 = nn.Dropout(self.drop_rate)
  ...
  def forward(self, inputs):
    # inputs.shape = (batch, 739, 1)
    results = self.layernorm1(inputs)
    results = self.dense1(results) # results.shape = (batch, 739, hidden_channels)
    results = self.gelu(results)
    results = self.dropout1(results)
    ...
    results = self.layernorm2(results)
    results = self.dense2(results) # results.shape = (batch, 739, hidden_channels)
    results = self.tanh(results) # results.shape = (batch, 739, hidden_channels)
