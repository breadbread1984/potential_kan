...
class EPWDataset(Dataset):
  ...
  def __len__(self):
    return self.data_count
  def __getitem__(self, index):
    memmap_index = bisect(self.start_indeces, index) - 1
    index_in_memmap = index - self.start_indices[memmap_index]
    rho = self.npys[memmap_index]['rho_inv_4_norm'][0,index_in_memmap] # data.shape = (75, 302,)
    exc = self.npys[memmap_index]['exc1_tr_lda'][index_in_memmap] # exc.shape = (75, 302,)
    weights = self.npys[memmap_index]['weights'][index_in_memmap] # weights.shape = (75, 302,)
    rho = np.ascontiguousarray(rho)
    energy = np.sum(rho * exc * weights)
    return rho.astype(np.float32), energy.astype(np.float32)
