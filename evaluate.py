...
def main(unused_argv):
  ...
  e_true = np.squeeze(np.concatenate(e_true, axis = 0)).detach().cpu().numpy()
  e_diff = np.squeeze(np.concatenate(e_diff, axis = 0)).detach().cpu().numpy()
  v_true = np.squeeze(np.concatenate(v_true, axis = 0)).detach().cpu().numpy()
  v_diff = np.squeeze(np.concatenate(v_diff, axis = 0)).detach().cpu().numpy()
